...
- 매도 조건
    - 매수 다음 영업일 매도
    - `gapRiseKeep` 설정에 따라 매도 방식이 결정됨 
      - false: 동시호가 시간에 매도. 동시호가 직전 예상 체결가 보다 낮게 매도 가격을 작성해 항상 메도 될 수 있또록 함
      - true: 상황별로 설명
        - 동시호가 직전 전날 영업일 종가보다 예상 체결가가 낮으면 동시호가 매도
        - 그렇지 않으면 매 5분마다 직전 5분봉 판단해 '시가 >= 종가' 이며 매도, 아니면 유지
