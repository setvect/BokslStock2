...
start() {
  ...
  nohup $JAVA -Dlogging.config=../conf/logback-spring.xml -Dspring.profiles.active=local -jar ../lib/BokslStock2-0.0.3.jar --spring.config.location=file:../conf/BokslStock2.yml 1> /dev/null 2>&1 &
  echo $! > ${RUNNING_PID}
...
console() {
  $JAVA -Dlogging.config=../conf/logback-spring.xml -Dspring.profiles.active=local -jar ../lib/BokslStock2-0.0.3.jar --spring.config.location=file:../conf/BokslStock2.yml
}
